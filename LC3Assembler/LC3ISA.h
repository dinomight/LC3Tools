...
namespace LC3
{
	...
	const unsigned int NUM_OPCODES = 25;
...
#include "LC3ISA.def"
	...
	//*** ISA Lexing ***
		...
		enum OpcodeEnum {ADD = 0, AND, BR, BRp, BRz, BRzp, BRn, BRnp, BRnz, BRnzp, JSR, JSRR, TRAP, JMP, LEA, LD, LDI, LDR, ST, STI, STR, NOT, RET, RTI, NOP};
	...
	//*** ISA Instructions ***
		...
		public:
			...
			/******************************************************************\
				...
				GetSources/Destinations will reflect PSR (NZP) use so that
				branches will be dependent on the previous ALU op.
				Returns the number of sources/destinations.
		...
		public:
			...
			virtual void AssignValues(vector<Number *>::iterator &, const vector<Number *>::iterator &);
			virtual bool GetImage(RamVector &, bool, CallBackFunction) const;
			virtual unsigned int GetSources(vector<RegisterEnum> &vReg) const;
			virtual bool IsBranch() const;
			virtual operator const char *() const;
		};
		...
		public:
			...
			/******************************************************************\
				LdInstr( [in] location stack, [in] Load type, [in] DR,
					[in] SOffset9 )

				Creates a load instruction. The final value could be a numerical
				index or a Symbol.
			\******/
			LdInstr(const LocationVector &, OpcodeEnum, RegisterEnum, Number *);
			virtual ~LdInstr();
		...
		public:
			...
			/******************************************************************\
				LdrInstr( [in] location stack, [in] DR, [in] BaseR,
					[in] SOffset6 )

				Creates a load instruction. The final value could be a numerical
				index or a Symbol.
			\******/
			LdrInstr(const LocationVector &, RegisterEnum, RegisterEnum, Number *);
			virtual ~LdrInstr();
		...
		class StInstr : public Instruction
		{
		public:
			RegisterEnum SR;
			Number *pSOffset9;
			...
			/******************************************************************\
				StInstr( [in] location stack, [in] store type, [in] SR,
					[in] SOffset9 )

				Creates a store instruction. The final value could be a
				numerical index or a Symbol.
			\******/
			StInstr(const LocationVector &, OpcodeEnum, RegisterEnum, Number *);
			virtual ~StInstr();
		...
		public:
			...
			/******************************************************************\
				StrInstr( [in] location stack, [in] SR, [in] BaseR,
					[in] SOffset6 )

				Creates a store instruction. The final value could be a
				numerical index or a Symbol.
			\******/
			StrInstr(const LocationVector &, RegisterEnum, RegisterEnum, Number *);
			virtual ~StrInstr();
