...
Fl_Menu_Item MemoryBytesWindow::MenuItems[] =
	{
		{ "&Memory Bytes", 0, 0, 0, FL_SUBMENU },
...
*/
Fl_Text_Display::Style_Table_Entry MemoryBytesWindow::StyleTable[] = 
	{	// Style table
		...
		{ FL_DARK_CYAN,		FL_COURIER,			14 }	// Y - Address
	};

const int Fudge = 54;

MemoryBytesWindow::MemoryBytesWindow() : Fl_Double_Window(300+Fudge, 450)
{
	{
		//Setup the menu
		...
		//Setup the memory bytes display
		...
		pLineNumber = new Fl_Input(220+Fudge, 28, 80, 20, "Line Number");
		...
		pMemoryName = new Fl_Choice(220+Fudge, 28+20, 80, 20, "Memory");
		...
		pLength->value("128");
		pEnter = new Fl_Return_Button(220+Fudge, 28+20*3+16, 80, 20, "Display");
...
MemoryBytesWindow::~MemoryBytesWindow()
{
	hide();
	SimulatorWindow::MemoryBytesMap::iterator MemoryIter = TheSimulatorWindow.MemoryBytes.find(this);
	if(MemoryIter == TheSimulatorWindow.MemoryBytes.end())
		throw "MemoryBytes does not contain this window!";
	TheSimulatorWindow.MemoryBytes.erase(MemoryIter);
	Fl::first_window()->show();
}

void MemoryBytesWindow::HelpCB(Fl_Widget *pW, void *pV)
{
	TheMainWindow.Help(AshIDEIndex, "MemoryBytesWindow");
}

bool MemoryBytesWindow::Close()
{
	Fl::delete_widget(this);
	return true;
}

bool MemoryBytesWindow::SetTitle()
{
	string sTitle = "Memory Bytes";

	label(sTitle.c_str());
	return true;
}

void MemoryBytesWindow::resize(int x, int y, int width, int height)
{
	//Reset the console width so that data/memory array displays fit in the message window
	if(width-4 > 0)
		MemoryBytesCW = MAX(MIN((width-26)/8, MAX_CONSOLE_WIDTH), MIN_CONSOLE_WIDTH);
	Fl_Double_Window::resize(x, y, width, height);
}

bool MemoryBytesWindow::ChangeInput()
{
...
bool MemoryBytesWindow::Enter()
{
	...
	if(pLineInput->value())
	{
		...
		string sCommand;
		sCommand = string("dla ")+sLineNumber+" "+sLength;
		TheSimulatorWindow.RedirectWindow = SimulatorWindow::SimMemoryBytesWindow;
		TheSimulatorWindow.pRedirectWindow = this;
		pTextDisplay->Clear();
		unsigned int CW = ConsoleWidth;
		TheSim(SetConsoleWidth(MemoryBytesCW));
		fRetVal = SIM_COMMAND(sCommand);
		TheSim(SetConsoleWidth(CW));
	...
	else if(pMemoryInput->value())
	{
		...
		string sCommand;
		sCommand = string("dma ")+sMemoryName+" "+sDataAddress+" "+sLength;
		TheSimulatorWindow.RedirectWindow = SimulatorWindow::SimMemoryBytesWindow;
		TheSimulatorWindow.pRedirectWindow = this;
		pTextDisplay->Clear();
		unsigned int CW = ConsoleWidth;
	...
	else
	{
		...
		string sCommand;
		sCommand = string("dda ")+sDataAddress+" "+sLength;
		TheSimulatorWindow.RedirectWindow = SimulatorWindow::SimMemoryBytesWindow;
		TheSimulatorWindow.pRedirectWindow = this;
		pTextDisplay->Clear();
		unsigned int CW = ConsoleWidth;
	...
	if(!fRetVal)
	{
		fl_alert("Error displaying memory.");
		show();
	}
	return fRetVal;
}

bool MemoryBytesWindow::ViewMemory(unsigned int ProgramNumber, const string &sIdentifier)
{
	pDataInput->value(1);
	ChangeInput();
...
bool MemoryBytesWindow::UpdateStyle(int Pos, int Inserted, int Deleted, int Restyled, const char *pszDeleted)
{
	int	Start, End;
	...
	delete [] pText;
	//The style buffer is already created by SimulatorWindow message functions
...
bool MemoryBytesWindow::ParseStyle(const string &sText, string &sStyle)
{
	string::size_type i, TempLoc1, TempLoc2;

	//look for colons
	TempLoc1 = sText.find(':');
	if(TempLoc1 == string::npos)
		return true;
	if(TempLoc1 != 0 && sStyle[TempLoc1-1] != 'A')
		//This line was already styled
		return true;
	TempLoc2 = sText.find(':', TempLoc1+1);
	if(TempLoc2 == string::npos)
		return true;
	for(i = 0; i < TempLoc1; i++)
		sStyle[i] = 'Y';
	sStyle[TempLoc1] = 'J';
	for(i = TempLoc1 + 1; i < TempLoc2; i++)
		sStyle[i] = 'G';
	sStyle[TempLoc2] = 'J';
	for(i = TempLoc2 + 1; i < sText.size(); i++)
		sStyle[i] = 'E';
