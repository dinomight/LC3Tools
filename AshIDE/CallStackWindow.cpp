...
Fl_Menu_Item CallStackWindow::MenuItems[] =
	{
		{ "&Call Stack", 0, 0, 0, FL_SUBMENU },
...
*/
Fl_Text_Display::Style_Table_Entry CallStackWindow::StyleTable[] = 
	{	// Style table
...
CallStackWindow::CallStackWindow() : Fl_Double_Window(400, 200)
{
	{
		//Setup the menu
		pMainMenu = new Fl_Menu_Bar(0, 0, w(), 28);
		pMainMenu->copy(MenuItems, this);
		//Setup the CallStack display
...
{
	hide();
	TheSimulatorWindow.pCallStackWindow = NULL;
	Fl::first_window()->show();
}

CallStackWindow::ReadOnlyEditorX::ReadOnlyEditorX(int X, int Y, int Width, int Height, Fl_Text_Display::Style_Table_Entry *pStyleTable, unsigned int nStyles) : ReadOnlyEditor(X, Y, Width, Height, pStyleTable, nStyles)
{
}

int CallStackWindow::ReadOnlyEditorX::handle(int Event)
{
...
bool CallStackWindow::ReadOnlyEditorX::GetFileLine(string &sFileName, unsigned int &LineNumber)
{
...
void CallStackWindow::HelpCB(Fl_Widget *pW, void *pV)
{
	TheMainWindow.Help(AshIDEIndex, "CallStackWindow");
}

bool CallStackWindow::Close()
{
	Fl::delete_widget(this);
	return true;
}

bool CallStackWindow::SetTitle()
{
	string sTitle = "Call Stack";

	label(sTitle.c_str());
	return true;
}

bool CallStackWindow::GetFileLine(string &sFileName, unsigned int &LineNumber)
{
	return pTextDisplay->GetFileLine(sFileName, LineNumber);
}

bool CallStackWindow::Enter()
{
	TheSimulatorWindow.RedirectWindow = SimulatorWindow::SimCallStackWindow;
	TheSimulatorWindow.pRedirectWindow = this;
	pTextDisplay->Clear();
	bool fRetVal = SIM_COMMAND("dcs");
	TheSimulatorWindow.RedirectWindow = SimulatorWindow::SimNoWindow;

	return fRetVal;
}

bool CallStackWindow::UpdateStyle(int Pos, int Inserted, int Deleted, int Restyled, const char *pszDeleted)
{
	int	Start, End;
...
bool CallStackWindow::ParseStyle(const string &sText, string &sStyle)
{
