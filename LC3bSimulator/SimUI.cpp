...
{
	ArchSim<LC3bISA> TheSim(SimMessageCallBack, SimCallBack, SimCommand, SimReadConsole, SimWriteConsole);
	LC3bArch TheArch(TheSim);

	char sMessageBuffer[64];
	SimCallBack(Info, "");
	sprintf(sMessageBuffer, "\t\tLC-3b Simulator %.15s, Ashley Wise", SIM_VER);
...
bool SimCommand(string &sCommand)
{
	...
	//Return the input
	sCommand = sTempBuff;
	sCommand = sCommand.substr(0, sCommand.find_first_of("\x0A\x0D\x04\xFF"));
...
bool LC3b::SimCommand(string &sCommand)
{
	if(fLeftOvers)
	{
		cin.seekg(0, ios::end);
...
bool LC3b::SimReadConsole(string &sBuffer, unsigned int CharsToRead, unsigned int &CharsRead)
{
	char sMessageBuffer[64];
	sprintf(sMessageBuffer, "<Simulating program requests %u character%.1s>", CharsToRead, (CharsToRead > 1 ? "s" : ""));
	SimCallBack(Info, sMessageBuffer);
